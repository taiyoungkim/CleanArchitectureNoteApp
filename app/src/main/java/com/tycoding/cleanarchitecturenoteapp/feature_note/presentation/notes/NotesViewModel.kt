package com.tycoding.cleanarchitecturenoteapp.feature_note.presentation.notes
...
class NotesViewModel @Inject constructor(
        ...
        when(event) {
            ...
            is NotesEvent.DeleteNote -> {
                viewModelScope.launch {
                    noteUserCases.deleteNote(event.note)
                    recentlyDeleteNote = event.note
                }
            }
            is NotesEvent.RestoreNote -> {
                viewModelScope.launch {
                    noteUserCases.addNote(recentlyDeleteNote ?: return@launch)
    ...
    private fun getNotes(noteOrder: NoteOrder) {
        getNotesJob?.cancel()
        getNotesJob = noteUserCases.getNotes(noteOrder)
            .onEach { notes ->
                _state.value = state.value.copy(
                    notes = notes,
                    noteOrder = noteOrder
